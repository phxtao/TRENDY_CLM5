...
for iworker = 1:worker_num
    ...
    while simu <= nsimu1
        ...
        if min(1, exp(-delta_cost*log(simu + 2))) > rand
            r2_soc = 1 - sum((obs_tot_soc_transient - mod_tot_soc_transient').^2)/sum((obs_tot_soc_transient - mean(obs_tot_soc_transient)).^2);
            r2_litter = 1 - sum((obs_tot_litter_transient - mod_tot_litter_transient').^2)/sum((obs_tot_litter_transient - mean(obs_tot_litter_transient)).^2);
            r2_cwd = 1 - sum((obs_tot_cwd_transient - mod_tot_cwd_transient').^2)/sum((obs_tot_cwd_transient - mean(obs_tot_cwd_transient)).^2);
            r2_hr = 1 - sum((obs_tot_hr_transient - mod_tot_hr_transient').^2)/sum((obs_tot_hr_transient - mean(obs_tot_hr_transient)).^2);
            ...
            disp([datestr(now,'HH:MM:SS'), ' upgrade1 iworker ', num2str(iworker), ': ', num2str(upgrade1), ' out of ', num2str(simu), ' cost: ', num2str(cost_new), ...
                ' r2: soc ', num2str(r2_soc), ' litter ', num2str(r2_litter), ' cwd ', num2str(r2_cwd), ' hr ', num2str(r2_hr)]);
    ...
    while simu <= nsimu2
        ...
        if mod(simu, 1000) == 0 || simu == nsimu2
            fun_save_da(data_path, sd_controling_factor, iworker, soc_mod_trace, litter_mod_trace, cwd_mod_trace, hr_mod_trace, parameters_keep2)
        end

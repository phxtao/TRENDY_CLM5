...
decom_fire_steady_state = frocing_steady_state.COL_FIRE_CLOSS*secspday./(frocing_steady_state.TOTLITC + frocing_steady_state.CWDC + frocing_steady_state.TOTVEGC); %unit day-1
...
decom_fire_transient = frocing_transient.COL_FIRE_CLOSS*secspday./(frocing_transient.TOTLITC + frocing_transient.CWDC + frocing_transient.TOTVEGC); %unit day-1
...
xi_fire_litter = kp(21)*(2 - 0) + 0;
xi_fire_cwd = kp(22)*(2 - 0) + 0;

% allocation to different litter pools
p4cwd_initial = kp(23)*(1 - 0.4) + 0.4;
p4cwd_end = kp(24)*(1 - 0) + 0;
...
%----------------------------------------
% Triangle Matrix, A Matrix, K Matrix, , Fire Matrix
%----------------------------------------
...
for imonth = 1:month_num
    ...
    % fire matrix
    fire_vector = [xi_fire_cwd*decom_fire_steady_state(imonth)*ones(n_soil_layer, 1); xi_fire_litter*decom_fire_steady_state(imonth)*ones(3*n_soil_layer, 1); zeros(3*n_soil_layer, 1)];
    fire_ma_middle(:, :, imonth) = diag(fire_vector);
end
tri_ma = mean(tri_ma_middle, 3, 'omitnan');
kk_ma = mean(kk_ma_middle, 3, 'omitnan');
fire_ma = mean(fire_ma_middle, 3, 'omitnan');
%----------------------------------------
% Vertical Profile
%----------------------------------------
...
for iyear = 1 : year_size
    for imonth = 1 : month_num
        ...
        % fire matrix
        fire_vector = [xi_fire_cwd*decom_fire_transient(month_loc)*ones(n_soil_layer, 1); xi_fire_litter*decom_fire_transient(month_loc)*ones(3*n_soil_layer, 1); zeros(3*n_soil_layer, 1)];
        fire_ma = diag(fire_vector);
